    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        ...
        builder.then(literal("save").executes(ctx -> {
            if (!mc.player.getMainHandStack().isEmpty()){
                ...
                if (!Files.exists(Paths.get("nerd-hack/SavedWorldInfo/"+serverip+"/WorldInfoData.txt"))){
                    File file = new File("nerd-hack/SavedWorldInfo/"+serverip+"/WorldInfoData.txt");
                    try {
                        file.createNewFile();
                    } catch (IOException e) {}
                }
                try {
                    new File("nerd-hack/SavedWorldInfo/"+serverip+"/").mkdirs();
                    FileWriter writer = new FileWriter("nerd-hack/SavedWorldInfo/"+serverip+"/WorldInfoData.txt", true);
                    if (!isNewGeneration) {
