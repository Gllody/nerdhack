...
public class AddonManager {
    ...
    public static void init() {
        // Meteor pseudo addon
        {
            NerdHack.ADDON = new MeteorAddon() {
                @Override
                ...
                public String getCommit() {
                    String commit = NerdHack.MOD_META.getCustomValue(NerdHack.MOD_ID + ":commit").getAsString();
                    return commit.isEmpty() ? null : commit;
                }
            };

            ModMetadata metadata = FabricLoader.getInstance().getModContainer(NerdHack.MOD_ID).get().getMetadata();

            NerdHack.ADDON.name = metadata.getName();
            NerdHack.ADDON.authors = new String[metadata.getAuthors().size()];
            if (metadata.containsCustomValue(NerdHack.MOD_ID + ":color")) {
                NerdHack.ADDON.color.parse(metadata.getCustomValue(NerdHack.MOD_ID + ":color").getAsString());
            }

            int i = 0;
            for (Person author : metadata.getAuthors()) {
                NerdHack.ADDON.authors[i++] = author.getName();
            }
        }
        ...
        for (EntrypointContainer<MeteorAddon> entrypoint : FabricLoader.getInstance().getEntrypointContainers("meteor", MeteorAddon.class)) {
            ...
            if (metadata.containsCustomValue(NerdHack.MOD_ID + ":color")) {
                addon.color.parse(metadata.getCustomValue(NerdHack.MOD_ID + ":color").getAsString());
            }
