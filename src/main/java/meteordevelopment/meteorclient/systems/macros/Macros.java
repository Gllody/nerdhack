...
public class Macros extends System<Macros> implements Iterable<Macro> {
    ...
    public void add(Macro macro) {
        macros.add(macro);
        NerdHack.EVENT_BUS.subscribe(macro);
        save();
    }
    ...
    public void remove(Macro macro) {
        if (macros.remove(macro)) {
            NerdHack.EVENT_BUS.unsubscribe(macro);
            save();
        }
    }
    ...
    public Macros fromTag(NbtCompound tag) {
        for (Macro macro : macros) NerdHack.EVENT_BUS.unsubscribe(macro);

        macros = NbtUtils.listFromTag(tag.getList("macros", 10), Macro::new);

        for (Macro macro : macros) NerdHack.EVENT_BUS.subscribe(macro);
        return this;
    }
}
