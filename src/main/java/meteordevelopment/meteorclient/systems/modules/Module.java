...
public abstract class Module implements ISerializable<Module>, Comparable<Module> {
    ...
    public void toggle() {
        if (!active) {
            ...
            if (runInMainMenu || Utils.canUpdate()) {
                if (autoSubscribe) NerdHack.EVENT_BUS.subscribe(this);
                onActivate();
            }
        }
        else {
            if (runInMainMenu || Utils.canUpdate()) {
                if (autoSubscribe) NerdHack.EVENT_BUS.unsubscribe(this);
                onDeactivate();
            }
    ...
    public void sendToggledMsg() {
        if (Config.get().chatFeedback.get() && chatFeedback) {
            ...
            ChatUtils.sendMsg(this.hashCode(), Formatting.GRAY, "> (highlight)%s(default).java = %s(default)", title, isActive() ? Formatting.GREEN + "on" : Formatting.RED + "off");
        }
    }
