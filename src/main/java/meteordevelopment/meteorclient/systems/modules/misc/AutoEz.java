...
public class AutoEz extends Module {
    ...
    private final Setting<MessageMode> killMsgMode = sgKill.add(new EnumSetting.Builder<MessageMode>()
        ...
        .defaultValue(MessageMode.NerdHack)
        .build()
    );
    ...
    private void sendKillMessage() {
        switch (killMsgMode.get()) {
            case NerdHack -> {
                int num = r.nextInt(0, nerdhacke.length);
                if (num == lastNum) {
                    num = num < nerdhacke.length - 1 ? num + 1 : 0;
                }
                lastNum = num;
                messageQueue.add(0, new Message(nerdhacke[num].replace("%s", name == null ? "You" : name), true));
            }
            case Exhibition -> {
