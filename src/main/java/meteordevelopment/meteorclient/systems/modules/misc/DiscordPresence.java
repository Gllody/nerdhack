...
//Created by squidoodly edited by G
...
public class DiscordPresence extends Module {
    ...
    private final Setting<SelectMode> line1SelectMode = sgLine1.add(new EnumSetting.Builder<SelectMode>()
        ...
        .defaultValue(SelectMode.NerdHack)
        .build()
    );
    ...
    private final Setting<List<String>> line2Strings = sgLine2.add(new StringListSetting.Builder()
        ...
        .defaultValue("NerdHack 0.8.8", "{round(server.tps, 1)} TPS", "Playing on {server.difficulty} difficulty.", "{server.player_count} Players online")
    ...
    public DiscordPresence() {
        super(Categories.Misc, "discord-presence", "Displays NerdHack as your presence on Discord.");
    ...
    public void onActivate() {
        ...
        String largeText = "%s %s".formatted(NerdHack.NAME, NerdHack.VERSION);
        if (!NerdHack.DEV_BUILD.isEmpty()) largeText += " Dev Build: " + NerdHack.DEV_BUILD;
        ...
        if (Utils.canUpdate()) {
            ...
            if (line1Ticks >= line1UpdateDelay.get() || forceUpdate) {
                if (!line1Scripts.isEmpty()) {
                    ...
                    if (line1SelectMode.get() == SelectMode.NerdHack) {
            ...
            if (line2Ticks >= line2UpdateDelay.get() || forceUpdate) {
                if (!line2Scripts.isEmpty()) {
                    ...
                    if (line2SelectMode.get() == SelectMode.NerdHack) {
            ...
            if (!lastWasInMainMenu) {
                rpc.setDetails(NerdHack.NAME + " " + (NerdHack.DEV_BUILD.isEmpty() ? NerdHack.VERSION : NerdHack.VERSION + " " + NerdHack.DEV_BUILD));
                ...
                else if (mc.currentScreen instanceof WidgetScreen) rpc.setState("Browsing NerdHack's GUI");
    ...
    private enum SmallImage {
        MineGame("minegame", "NerdHack"),
        Snail("seasnail", "NerdHax");
