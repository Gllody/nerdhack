...
public class BaseFinder extends Module {
    ...
    public BaseFinder() {
        super(Categories.Misc,"BaseFinder", "Estimates if a build or base may be in the chunk based on the blocks it contains.");
    }
    @Override
    public void onActivate() {
        ...
        world= mc.world.getRegistryKey().getValue().toString().replace(':', '_');
        if (save.get()){
            new File("nerd-hack/BaseChunks/"+serverip+"/"+world).mkdirs();
        }
        if (load.get()){
            loadData();
        }
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        if (deletewarning>=2){
            baseChunks.clear();
            new File("nerd-hack/BaseChunks/"+serverip+"/"+world+"/BaseChunkData.txt").delete();
            closestbaseX=2000000000;
            closestbaseZ=2000000000;
            basedistance=2000000000;
            LastBaseFound= new ChunkPos(2000000000, 2000000000);
        ...
        if (!baseChunks.contains(new ChunkPos(AddCoordX,AddCoordZ))){
            ...
            try {
                new File("nerd-hack/BaseChunks/"+serverip+"/"+world).mkdirs();
                FileWriter writer = new FileWriter("nerd-hack/BaseChunks/"+serverip+"/"+world+"/BaseChunkData.txt", true);
    ...
    private void loadData() {
        try {
            List<String> allLines = Files.readAllLines(Paths.get("nerd-hack/BaseChunks/"+serverip+"/"+world+"/BaseChunkData.txt"));
