...
public class Modules extends System<Modules> {
    ...
    void addActive(Module module) {
        synchronized (active) {
            if (!active.contains(module)) {
                active.add(module);
                NerdHack.EVENT_BUS.post(ActiveModulesChangedEvent.get());
            }
        }
    }
    ...
    private boolean onBinding(boolean isKey, int value, int modifiers) {
        ...
        NerdHack.EVENT_BUS.post(ModuleBindChangedEvent.get(moduleToBind));
        moduleToBind = null;
        ...
        synchronized (active) {
            for (Module module : modules) {
                if (module.isActive() && !module.runInMainMenu) {
                    NerdHack.EVENT_BUS.subscribe(module);
                    module.onActivate();
                }
            }
        }
    ...
    private void onGameLeft(GameLeftEvent event) {
        synchronized (active) {
            for (Module module : modules) {
                if (module.isActive() && !module.runInMainMenu) {
                    NerdHack.EVENT_BUS.unsubscribe(module);
                    module.onDeactivate();
                }
            }
        }
    ...
    public static class ModuleRegistry extends SimpleRegistry<Module> {
        public ModuleRegistry() {
            super(RegistryKey.ofRegistry(NerdHack.identifier("modules")), Lifecycle.stable());
        }
