...
public class Step extends Module {
    public Step() {
        super(Categories.Movement, "NOStep", "NOT WORKING AT THIS MOMENT!");
    }
    ...
    public void step(double[] offsets) {
        ...
        for (double v : offsets) {
            offset += v;
            PacketCallbacks.of((Supplier<Packet<?>>) new PlayerMoveC2SPacket.PositionAndOnGround( mc.player.getX() +(0), mc.player.getY() + offset +(0), mc.player.getZ() +(0), false));
        }
        lastStep = System.currentTimeMillis();
    }
