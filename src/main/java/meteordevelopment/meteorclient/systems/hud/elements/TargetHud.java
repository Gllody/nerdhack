...
import meteordevelopment.meteorclient.systems.blackoutfiles.RenderUtilsss;
...
public class TargetHud extends HudElement {
    ...
    private final Setting<SettingColor> healthColor = sgGeneral.add(new ColorSetting.Builder()
        ...
        .description(NerdHack.COLOR)
        .defaultValue(new SettingColor(59, 250, 92, 255))
        .build()
    );
    ...
    public static final HudElementInfo<TargetHud> INFO = new HudElementInfo<>(Hud.GROUP, "TargetHud", "A target hud the fuck you thinkin bruv.", TargetHud::new);
    ...
    final Identifier backgroundofhacker = NerdHack.identifier("textures/misaka10032bynekojdm.png");
    final Identifier backgroundofarch = NerdHack.identifier("textures/archlinuxwallpaperforbg.png");
    final Identifier backgroundofarchtwo = NerdHack.identifier("textures/archlinuxnicesecondbg.png");
    final Identifier heckerbgtopvplol = NerdHack.identifier("textures/heckerbgtopvp.png");

    @Override
    public void render(HudRenderer renderer) {
        if (mode.get() == Mode.Nerdhack) {
            int height = 60;
            int width = 190;
            ...
            GL.bindTexture(backgroundofhacker);
            Renderer2D.TEXTURE.begin();
            Renderer2D.TEXTURE.texQuad(x,y, scale.get() * 190, scale.get() * 60, new Color(255, 255, 255, 255));
            ...
            // Background
            RenderUtilsss.quad(stack, -2, -2, width + 4, height + 4, new Color(52, 52, 52, 255).getPacked());
            RenderUtilsss.quad(stack, -1, -1, width + 2, height + 2, new Color(32, 32, 32, 255).getPacked());
            RenderUtilsss.quad(stack, 0, 0, width, height, new Color(52, 52, 52, 255).getPacked());

            //PlayerModel

            // Name
            stack.scale(1.5f,1.5f,1);
            RenderUtilsss.text(renderName, stack, 11, 2, textColor.get().getPacked());

            RenderUtilsss.quad(stack, 1, 1, 58, 58, new Color(102, 102, 102, 255).getPacked());

            drawFace(renderer, scale.get().floatValue(), x, y, 0);

            // Health and Distance
            stack.scale(0.8f,0.8f,1);
            RenderUtilsss.text(Math.round((renderHealth) * 10) / 10f + " Dist: " + Math.round(mc.player.distanceTo(target) * 10) / 10f, stack, 50, 25 - mc.textRenderer.fontHeight / 2f, healthColor.get().getPacked());
            ...
            for (int i = 0; i < 10; i++) {
                RenderUtilsss.quad(stack, 41 + i * 8, 12, 3 * Math.min(progress, 2), 3, new Color(204, 204, 0, 255).getPacked());
                progress -= 2;
        ...
        if (mode.get() == Mode.WorldWar) {
            ...
            GL.bindTexture(backgroundofarch);
            Renderer2D.TEXTURE.begin();
            Renderer2D.TEXTURE.texQuad(x,y, scale.get() * 190, scale.get() * 60, new Color(255, 255, 255, 255));
            Renderer2D.TEXTURE.render(matrixStack);
            ...
            // Name
            stack.scale(1.5f,1.5f,1);
            RenderUtilsss.text(renderName, stack, 10, 2, textColor.get().getPacked());
            ...
            renderer.drawContext.drawItem(itemStack, 102, 5);

            drawStack.pop();
            // gayer model

        }
        if (mode.get() == Mode.ArchLinux) {
            int height = 60;
            int width = 190;
            setSize(width * scale.get(), height * scale.get());
            ...
            GL.bindTexture(backgroundofarchtwo);
            Renderer2D.TEXTURE.begin();
            Renderer2D.TEXTURE.texQuad(x,y, scale.get() * 190, scale.get() * 60, new Color(255, 255, 255, 255));
            Renderer2D.TEXTURE.render(matrixStack);
        ...
        if (mode.get() == Mode.Hecker) {
            ...
            GL.bindTexture(heckerbgtopvplol);
            Renderer2D.TEXTURE.begin();
            Renderer2D.TEXTURE.texQuad(x,y, scale.get() * 190, scale.get() * 60, new Color(255, 255, 255, 255));
            Renderer2D.TEXTURE.render(matrixStack);
        ...
        if (mode.get() == Mode.Blackout) {
            ...
            // Background
            RenderUtilsss.rounded(stack, 15, 15, width - 30, height - 30, 15, 10, bgColor.get().getPacked());
            ...
            // Name
            RenderUtilsss.text(renderName, stack, 60, 20, textColor.get().getPacked());

            // Ping
            RenderUtilsss.text(Math.round(renderPing) + "ms", stack, 60, 30, textColor.get().getPacked());

            // Health
            RenderUtilsss.text(String.valueOf(Math.round((renderHealth) * 10) / 10f), stack, 20, 81 - mc.textRenderer.fontHeight / 2f, textColor.get().getPacked());
            ...
            // Health Bar
            if (barAnimation > 0) {
                RenderUtilsss.rounded(stack, barStart, 80, MathHelper.clamp(barAnimation / 20, 0, 1) * (width - 30 - barStart), 2, 3, 10, healthColor.get().getPacked());
            }
            if (barAnimation > 20) {
                RenderUtilsss.rounded(stack, barStart, 80, MathHelper.clamp((barAnimation - 20) / 16, 0, 1) * (width - 30 - barStart), 2, 3, 10, absorptionColor.get().getPacked());
            }
        }
        if (mode.get() == Mode.ExhibitionOld) {
            ...
            // Background
            RenderUtilsss.quad(stack, 0, 0, width, height, bgColor.get().getPacked());
            ...
            RenderUtilsss.text(renderName, stack, 33, 2, textColor.get().getPacked());

            // Health
            stack.scale(0.5f,0.5f,1);
            RenderUtilsss.text(Math.round((renderHealth) * 10) / 10f + " Dist: " + Math.round(mc.player.distanceTo(target) * 10) / 10f, stack, 66, 35 - mc.textRenderer.fontHeight / 2f, textColor.get().getPacked());
            ...
            for (int i = 0; i < 10; i++) {
                RenderUtilsss.quad(stack, 33 + i * 8, 11, 3 * Math.min(progress, 2), 3, new Color(204, 204, 0, 255).getPacked());
                progress -= 2;
            ...
            stack.scale(0.5f,0.5f,1);
            // Misc info
            RenderUtilsss.text("Yaw: " + Math.round((target.getYaw()) * 10) / 10f + " Pitch: " + Math.round(target.getPitch() * 10) / 10f + " BodyYaw: " + Math.round((target.getBodyYaw()) * 10) / 10f , stack, 66, 45 - mc.textRenderer.fontHeight / 2f, textColor.get().getPacked());
            RenderUtilsss.text("TOG: " + (tog.getOrDefault(target, 0)) + " HURT: " + ((target.hurtTime) * 10) / 10f + " TE: " + target.age, stack, 66, 55 - mc.textRenderer.fontHeight / 2f, textColor.get().getPacked());
        }
        if (mode.get() == Mode.Exhibition){
            ...
            // Name
            stack.scale(1.5f,1.5f,1);
            RenderUtilsss.text(renderName, stack, 41, 2, textColor.get().getPacked());
            ...
            // Health and Distance
            stack.scale(0.5f,0.5f,1);
            RenderUtilsss.text(Math.round((renderHealth) * 10) / 10f + " Dist: " + Math.round(mc.player.distanceTo(target) * 10) / 10f, stack, 83, 40 - mc.textRenderer.fontHeight / 2f, textColor.get().getPacked());
