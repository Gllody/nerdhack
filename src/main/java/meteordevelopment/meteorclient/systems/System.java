...
public abstract class System<T> implements ISerializable<T> {
    ...
    public System(String name) {
        ...
        if (name != null) {
            this.file = new File(NerdHack.FOLDER, name + ".nbt");
    ...
    public void save(File folder) {
        ...
        try {
            File tempFile = File.createTempFile(NerdHack.MOD_ID, file.getName());
        ...
        try {
            ...
            if (file.exists()) {
                try {
                    ...
                    NerdHack.LOG.error("Error loading " + this.name + ". Possibly corrupted?");
                    NerdHack.LOG.info("Saved settings backup to '" + backup + "'.");
                    e.printStackTrace();
                }
            }
        } catch (IOException e) {
