...
import meteordevelopment.meteorclient.systems.blackoutfiles.IRaycastContextBlackout;
import meteordevelopment.meteorclient.systems.blackoutfiles.meteor.BODamageUtils;
...
public class RaytraceSettings extends Module {
    public RaytraceSettings() {
        super(Categories.Player, "Raytrace", "Global raytrace settings for every blackout module.");
    }
    ...
    public boolean placeTrace(BlockPos pos) {
        ...
        switch (placeMode.get()) {
            case SinglePoint -> {
                ((IRaycastContextBlackout) raycastContext).setEnd(new Vec3d(pos.getX() + 0.5, pos.getY() + placeHeight.get(), pos.getZ() + 0.5));

                result = BODamageUtils.raycast(raycastContext);
                return result.getBlockPos().equals(pos);
            }
            case DoublePoint -> {
                ((IRaycastContextBlackout) raycastContext).setEnd(new Vec3d(pos.getX() + 0.5, pos.getY() + placeHeight1.get(), pos.getZ() + 0.5));
                ...
                ((IRaycastContextBlackout) raycastContext).setEnd(new Vec3d(pos.getX() + 0.5, pos.getY() + placeHeight2.get(), pos.getZ() + 0.5));
            ...
            case Sides -> {
                ...
                for (Direction dir : Direction.values()) {
                    ((IRaycastContextBlackout) raycastContext).setEnd(vec.add(dir.getOffsetX() / 2f, dir.getOffsetY() / 2f, dir.getOffsetZ() / 2f));
            ...
            case Exposure -> {
                ...
                for (int x = 0; x <= 2; x += 1) {
                    for (int y = 0; y <= 2; y += 1) {
                        for (int z = 0; z <= 2; z += 1) {
                            ((IRaycastContextBlackout) raycastContext).setEnd(vec.add(0.1 + x * 0.4, 0.1 + y * 0.4, 0.1 + z * 0.4));
        ...
        switch (attackMode.get()) {
            ...
            case Exposure -> {
                ...
                for (int x = 0; x <= 2; x += 1) {
                    for (int y = 0; y <= 2; y += 1) {
                        for (int z = 0; z <= 2; z += 1) {
                            ((IRaycastContextBlackout) raycastContext).setEnd(vec.add(MathHelper.lerp(x / 2f, 0.1, xw - 0.1), MathHelper.lerp(y / 2f, 0.0, yh - 0.1), MathHelper.lerp(z / 2f, 0.1, zw - 0.1)));
    ...
    private void updateContext() {
        if (raycastContext == null) {
            ...
            ((IRaycastContextBlackout) raycastContext).setStart(mc.player.getEyePos());
        }
    }
}
