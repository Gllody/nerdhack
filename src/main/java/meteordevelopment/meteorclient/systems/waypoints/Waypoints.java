...
public class Waypoints extends System<Waypoints> implements Iterable<Waypoint> {
    public static final String[] BUILTIN_ICONS = {"nerd", "clown", "thinking", "heart", "jerry", "doge", "hehe", "heker", "square", "circle", "triangle", "star", "swords", "skull"};
    ...
    @Override
    public void init() {
        File iconsFolder = new File(new File(NerdHack.FOLDER, "waypoints"), "icons");
        iconsFolder.mkdirs();
    ...
    public File getFile() {
        ...
        return new File(new File(NerdHack.FOLDER, "waypoints"), Utils.getFileWorldName() + ".nbt");
    }
    ...
    private void copyIcon(File file) {
        StreamUtils.copy(Waypoints.class.getResourceAsStream("/assets/" + NerdHack.MOD_ID + "/textures/icons/waypoints/" + file.getName()), file);
    }
