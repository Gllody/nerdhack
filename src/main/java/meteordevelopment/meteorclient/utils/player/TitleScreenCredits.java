...
public class TitleScreenCredits {
    ...
    private static void init() {
        ...
        credits.sort(Comparator.comparingInt(value -> value.addon == NerdHack.ADDON ? Integer.MIN_VALUE : -mc.textRenderer.getWidth(value.text)));
            ...
            for (Credit credit : credits) {
                ...
                switch (res.statusCode()) {
                    case Http.UNAUTHORIZED -> {
                        ...
                        NerdHack.LOG.warn(message);
                        if (System.getenv("meteor.github.authorization") == null) {
                            NerdHack.LOG.info("Consider setting1 an authorization " +
                                "token with the 'meteor.github.authorization' environment variable.");
                            NerdHack.LOG.info("See: https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens");
                        }
                    }
                    case Http.FORBIDDEN -> {
                        NerdHack.LOG.warn("Could not fetch updates for addon '%s': Rate-limited by GitHub.".formatted(credit.addon.name));
                    }
                    case Http.NOT_FOUND -> {
                        NerdHack.LOG.warn("Could not fetch updates for addon '%s': GitHub repository '%s' not found.".formatted(credit.addon.name, repo.getOwnerName()));
                    }
                    case Http.SUCCESS -> {
    ...
    private static void add(MeteorAddon addon) {
        ...
        credit.text.append(Text.literal(" by ").formatted(Formatting.GREEN));
        ...
        for (int i = 0; i < addon.authors.length; i++) {
            ...
            credit.text.append(Text.literal(addon.authors[i]).formatted(Formatting.YELLOW));
        }
